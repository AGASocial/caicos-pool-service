...
async function rotateUserKey() {
    ...
    // 1. Get Old Key
    const { data: user, error } = await supabaseAdmin.from('cadenza_profiles').select('encrypted_storage_key').eq('id', USER_ID).single();
...
async function updateUserKey(newKey: Buffer) {
    ...
    await supabaseAdmin.from('cadenza_profiles').update({ encrypted_storage_key: encrypted }).eq('id', USER_ID);
