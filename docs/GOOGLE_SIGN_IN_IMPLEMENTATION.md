...
This document provides step-by-step instructions for implementing Google Sign In authentication in the cadenza-app project using Supabase Auth.
...
3. Click **"New Project"**
4. Enter a project name (e.g., "cadenza-app")
5. Click **"Create"**
...
4. Fill in the required information:
   - **App name**: cadenza-app (or your app name)
...
4. Give it a name (e.g., "cadenza-app-web")
...
if (session?.user) {
  ...
  if (!existingUser) {
    // Create user record
    await supabase.from('cadenza_profiles').insert({
      id: session.user.id,
